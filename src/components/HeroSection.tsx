    ...
    <section className="relative bg-gradient-to-br from-pink-50 via-pink-25 to-white h-[70vh] md:h-[80vh] flex items-center overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex items-center justify-center h-full">
          {/* Centered Content */}
          <div className="text-center max-w-2xl pb-16 md:pb-0">
            {/* Main Heading */}
            ...
            {/* Subtitle */}
            <p className="text-white/90 text-sm md:text-lg lg:text-xl mb-4 md:mb-6 max-w-lg mx-auto drop-shadow-md">
            ...
            <Link to="/about">
              <Button 
                size="lg" 
                className="bg-gradient-to-r from-pink-400 to-pink-300 hover:from-pink-500 hover:to-pink-400 text-white px-6 md:px-10 py-2 md:py-4 rounded-full font-semibold text-sm md:text-lg shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 border border-pink-200"
              >
                About Us
              </Button>
            </Link>
      ...
      <div className="absolute bottom-2 md:bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
        <div className="flex flex-col items-center text-white/70">
          <p className="text-xs md:text-sm mb-1 md:mb-2 font-medium">Shop Below</p>
          <ChevronDown className="w-5 h-5 md:w-8 md:h-8 animate-pulse" />
          <div className="w-4 h-6 md:w-6 md:h-10 border-2 border-white/50 rounded-full flex justify-center mt-1 md:mt-2">
            <div className="w-1 h-1.5 md:h-3 bg-white/70 rounded-full mt-1 md:mt-2"></div>
          </div>
        </div>
      </div>
    </section>
